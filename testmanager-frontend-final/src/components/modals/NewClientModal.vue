...
const save = async () => {
  ...
  try {
    saving.value = true
    // createCredentials: true garante que o backend crie o usuário do auth
    await api.post('/api/admin/clients', { ...form, createCredentials: true })
    emit('created')
    emit('close')
  } catch (e) {
    passwordError.value = e?.response?.data?.error || 'Erro ao criar cliente'
  } finally {
    saving.value = false
  }
}
